...
if (fs.existsSync(`${projectCWD}/${pathArg}${compoentFileType}`)) {
  console.log('component has existed');
  process.exit(1);
}
...
if (pathArg.split('/').length > 1) {
  ...
  const targetFloder = scriptCWD.startsWith(projectCWD) ? scriptCWD : `${projectCWD}/${folderPath}`;
  if (fs.existsSync(targetFloder)) {
    console.log('文件夹已经存在');
  } else {
    fs.mkdirSync(scriptCWD.startsWith(projectCWD) ? scriptCWD : `${projectCWD}/${folderPath}`);
  }
}
...
if (pathArg.includes('/')) {
  if (fs.existsSync(compoentPath.slice(0, compoentPath.lastIndexOf('/')))) {
    console.log('文件夹已经存在');
  } else {
    fs.mkdirSync(compoentPath.slice(0, compoentPath.lastIndexOf('/')));
  }
}
